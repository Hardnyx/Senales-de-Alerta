...
Private Function M_MC_RAW(ByVal rutaArchivo As String) As String
    ...
    MLine m, "        posD = if t2=null then -1 else Text.PositionOf(t2, ""."", Occurrence.Last),"
    MLine m, "        posC = if t2=null then -1 else Text.PositionOf(t2, "","", Occurrence.Last),"
    MLine m, "        t3 = if t2=null then null"
    MLine m, "             else if (posD >= 0) and (posC >= 0) then (if posC < posD"
    MLine m, "                 then Text.Replace(t2, "","", """")"
    MLine m, "                 else Text.Replace(Text.Replace(t2,""."",  """"),"","",""."")),"
    MLine m, "             else if (posC >= 0) and (posD < 0) then Text.Replace(t2,"","","".""),"
    MLine m, "             else t2,"
