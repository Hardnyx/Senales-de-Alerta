...
Private Function M_MC_MAIN(ByVal mesesSel As Long) As String
    ...
    ' Parseo de fechas DDMMMYYYY optimizado:
    ' En lugar de una funcion custom row-by-row (lento para 60k+ filas),
    ' se hacen 12 reemplazos bulk sobre la columna completa con Table.ReplaceValue,
    ' luego un AddColumn bulk construye el ISO string, y TransformColumnTypes parsea en bloque.
    ' Esto es O(12 * n) pasadas bulk vs O(n) custom function calls con overhead de PQ.
    MLine m, "  WithFecha ="
    MLine m, "    if not List.Contains(Table.ColumnNames(Ren), ""Fecha"") then Ren"
    MLine m, "    else"
    MLine m, "      let"
    MLine m, "        // 12 reemplazos bulk: sustituye abreviatura ES por numero de mes"
    MLine m, "        R1  = Table.ReplaceValue(Ren,  ""ENE"", ""01"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R2  = Table.ReplaceValue(R1,   ""FEB"", ""02"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R3  = Table.ReplaceValue(R2,   ""MAR"", ""03"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R4  = Table.ReplaceValue(R3,   ""ABR"", ""04"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R5  = Table.ReplaceValue(R4,   ""MAY"", ""05"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R6  = Table.ReplaceValue(R5,   ""JUN"", ""06"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R7  = Table.ReplaceValue(R6,   ""JUL"", ""07"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R8  = Table.ReplaceValue(R7,   ""AGO"", ""08"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R9  = Table.ReplaceValue(R8,   ""SET"", ""09"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R10 = Table.ReplaceValue(R9,   ""SEP"", ""09"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R11 = Table.ReplaceValue(R10,  ""OCT"", ""10"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R12 = Table.ReplaceValue(R11,  ""NOV"", ""11"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        R13 = Table.ReplaceValue(R12,  ""DIC"", ""12"", Replacer.ReplaceText, {""Fecha""}),"
    MLine m, "        // Fecha ahora: ""27092024"" (DDMMYYYY) -> construir ""2024-09-27"""
    MLine m, "        ISOTxt = Table.TransformColumns(R13, {{""Fecha"","
    MLine m, "          each let s = Text.From(_)"
    MLine m, "               in if Text.Length(s) = 8"
    MLine m, "                  then Text.End(s,4) & ""-"" & Text.Range(s,2,2) & ""-"" & Text.Start(s,2)"
    MLine m, "                  else null, type text}}),"
    MLine m, "        Typed = Table.TransformColumnTypes(ISOTxt, {{""Fecha"", type date}}, ""en-US"")"
    MLine m, "      in Typed,"
